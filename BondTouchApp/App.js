...
function MainScreen({ myName, topicName }) {
  ...
  const heartGlowAnim = useRef(new Animated.Value(0)).current;  // non-native: opacity only
  ...
  const lastSendTapTime = useRef(0);
  // ── SEND HEART TAP ──────────────────────────────────────────────────────────
  const sendHeartTap = useCallback(() => {
    if (!mqttRef.current) return;
    const now = Date.now();
    if (now - lastSendTapTime.current < 1000) return; // 1s throttle
    lastSendTapTime.current = now;
    ...
    // Heart press animation — scale uses native driver, glow uses non-native (separate views)
    Animated.sequence([
      Animated.timing(heartScale, { toValue: 0.88, duration: 100, useNativeDriver: true }),
      Animated.spring(heartScale, { toValue: 1, friction: 3, useNativeDriver: true }),
    ]).start();
    Animated.sequence([
      Animated.timing(heartGlowAnim, { toValue: 1, duration: 180, useNativeDriver: false }),
      Animated.timing(heartGlowAnim, { toValue: 0, duration: 500, useNativeDriver: false }),
    ]).start();
  }, [topicName, myName]);
  ...
  // Glow overlay style — non-native driver only, on its own separate Animated.View
      ...
      >
        ...
        <View style={s.touchSection}>
          ...
          <View style={s.touchPadWrap}>
            ...
            {/* Main heart button — scale on native driver Animated.View */}
            ...
            {/* Glow overlay — non-native driver, separate Animated.View (no conflict!) */}
            <Animated.View style={glowOverlayStyle} pointerEvents="none" />
          </View>
